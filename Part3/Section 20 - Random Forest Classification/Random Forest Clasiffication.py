...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 20 02:12:20 2020

@author: root
"""

#Plantilla de Clasificacion Random Forest
...
plt.title('Clasificador Random Forest(Conjunto de Entrenamiento)')
plt.xlabel('Edad')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Clasificador Random Forest(Conjunto de Test)')
plt.xlabel('Edad')
plt.ylabel('Sueldo Estimado')
plt.legend()
plt.show()
